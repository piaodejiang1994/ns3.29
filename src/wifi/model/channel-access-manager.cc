...
ChannelAccessManager::RequestAccess (Ptr<Txop> state, bool isCfPeriod)
{
  ...
  //NS_LOG_UNCOND ("request channel access");
  if (m_sleeping || m_off)
    {
      return;
    }
  ...
  if (lastTxEnd > Simulator::Now ())
    {
     ...
     //pdj NS_LOG_UNCOND ("request channel access - Internal collision (currently transmitting)");
  ...
  if (state->GetBackoffSlots () == 0)
    {
      if (IsBusy ())
       ...
       //pdj   NS_LOG_UNCOND ("request channel access - medium is busy: collision");
      ...
      else if (IsWithinAifs (state))
        {
          ...
          //pdj  NS_LOG_UNCOND ("request channel access - busy within AIFS");
          state->NotifyCollision ();
          DoRestartAccessTimeoutIfNeeded ();
          return;
        }
  ...
  //pdj  NS_LOG_UNCOND ("request channel access");
  DoGrantAccess ();
  DoRestartAccessTimeoutIfNeeded ();
}
